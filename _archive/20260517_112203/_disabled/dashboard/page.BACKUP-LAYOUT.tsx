...
export default function Dashboard() {
  ...
  const alerts = (data.results || []).map(r => {
    if (r.trend === 'falling') {
      return { text: r.partNumber + ' â†’ ðŸ”¥ Price dropping', type: 'good' }
    }
    if (r.trend === 'rising') {
      return { text: r.partNumber + ' â†’ âš ï¸ Price rising', type: 'warn' }
    }
    return null
  }).filter(Boolean)
...
cursor: 'pointer'
  ...
  <strong>{s.supplier}</strong>
            ...
            {s.best && <div style={{ fontWeight: '700', color: '#16a34a' }}>ðŸ’° BEST VALUE</div>}
  ...
  return (
    <div style={{ fontSize: '12px', marginTop: '6px', color: '#555' }}>
      ...
      <div style={{ color: '#facc15', marginTop: '4px' }}>
        {'â˜…'.repeat(s.rating || 4)}{'â˜†'.repeat(5 - (s.rating || 4))}
      </div>
...
{(() => {
  ...
  return (
    <div style={{
      ...
      <div>ðŸ“ {meta.distance}</div>
      <div>ðŸšš {meta.delivery}</div>
      {meta.online && <div style={{ color: '#2563eb' }}>ðŸŒ Online Only</div>}
    </div>
  )
})()}</div>
