...
$ExportStatements = New-Object System.Collections.Generic.List[string]

foreach ($Revision in $RevisionFiles) {

    Write-JDStatus ("Adding " + $Revision.Name)
    Write-JDBootstrapLog ("Reading " + $Revision.Name)

    $Content = Get-Content -LiteralPath $Revision.FullName -Raw

    [void]$Builder.AppendLine("#------------------------------------------------------------------------------")
    [void]$Builder.AppendLine("# BEGIN " + $Revision.Name)
    [void]$Builder.AppendLine("#------------------------------------------------------------------------------")

    $SkipExportBlock = $false
...
foreach($Line in ($Content -split "`r?`n")){
    ...
    if($Line -match '^\s*using\s+namespace\s+'){
        [void]$UsingStatements.Add($Line.Trim())
        continue
    }

    if($Line -match '^\s*Export-ModuleMember\b'){
        $ExportStatements.Add($Line.Trim())
        $SkipExportBlock = $true
        continue
    }
    ...
    [void]$Builder.AppendLine()
    [void]$Builder.AppendLine("#------------------------------------------------------------------------------")
    [void]$Builder.AppendLine("# END " + $Revision.Name)
    [void]$Builder.AppendLine("#------------------------------------------------------------------------------")
    [void]$Builder.AppendLine()
}

$ModuleBuilder = New-Object System.Text.StringBuilder

foreach($Using in ($UsingStatements | Sort-Object)){
    [void]$ModuleBuilder.AppendLine($Using)
}
[void]$ModuleBuilder.AppendLine()

[void]$ModuleBuilder.Append($Builder.ToString())

[void]$ModuleBuilder.AppendLine()
[void]$ModuleBuilder.AppendLine("# Export block emitted in WP-BUILD-009C")
