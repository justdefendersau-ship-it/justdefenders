...
#
# File      : tooling/engineering/JDEP/Public/Update-JDEPEngineeringDecisionRegister.ps1
# Programme : EP-001
# WorkPack  : PR-003
# Unit      : EU-002
#
...
function Update-JDEPEngineeringDecisionRegister {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [ValidateNotNull()]
    ...
    $decisionRegisterPath = Join-Path $documentationPath 'Engineering-Decision-Register.md'

    $today = (Get-Date).Date

    $lines = [System.Collections.Generic.List[string]]::new()

    $lines.Add('# JDEP Engineering Decision Register')
    $lines.Add('')
    $lines.Add('This document is generated from Engineering Policy evidence.')
    $lines.Add('Manual changes will be replaced.')
    $lines.Add('')
    $lines.Add('| Policy Identifier | Policy Title | Policy Category | Policy Authority | Effective Date | Review Date | Decision Status |')
    $lines.Add('|-------------------|--------------|-----------------|------------------|----------------|-------------|-----------------|')
    ...
    foreach ($policy in ($EngineeringPolicy | Sort-Object PolicyIdentifier)) {
        ...
        $lines.Add(
            "| $($policy.PolicyIdentifier) | $title | $category | $authority | $($effectiveDate.ToString('yyyy-MM-dd')) | $($reviewDate.ToString('yyyy-MM-dd')) | $status |"
        )
    ...
    [pscustomobject]@{
        DecisionRegisterPath = $decisionRegisterPath
...
Export-ModuleMember -Function Update-JDEPEngineeringDecisionRegister