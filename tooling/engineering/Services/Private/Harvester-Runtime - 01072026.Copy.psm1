...
if (Test-Path $privateFolder)
{
    ...
    ForEach-Object {
        ...
        try
        {
            Write-JDEngineeringLog `
                -Level Information `
                -Message ("Loading private module [{0}]." -f $file.Name)

            . $file.FullName
        }
        catch
        {
            throw "Failed loading private module [$($file.Name)]`n$($_.Exception | Out-String)"
        }
    ...
    Write-JDEngineeringLog `
        -Level Information `
        -Message "Private module loading complete."
}
